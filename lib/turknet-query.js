...
class TurknetQuery {
    ...
    async makeQuery(queryType, queryValue) {
        ...
        if (!res.FiberServiceAvailablity) res.FiberServiceAvailablity = {
            IsAvailable: false,
            IsGigaFiber: false,
            MaxCapacity: -1
        };

        if (!res.VAEFiberServiceAvailability) res.VAEFiberServiceAvailability = {
            IsAvailable: false,
            MaxCapacity: -1,
            MaxCapacityServiceType: -1,
            NmsMax: -1,
            Type: -1,
            Description: null
        };

        if (!res.VDSLServiceAvailability) res.VDSLServiceAvailability = {
            IsAvailable: false,
            MaxCapacity: -1,
            MaxCapacityServiceType: -1,
            NmsMax: -1,
        ...
        if (!res.XDSLServiceAvailability) res.XDSLServiceAvailability = {
            IsAvailable: false,
            MaxCapacity: -1,
            NmsMax: -1,
            Description: null
        ...
        if (!res.YapaServiceAvailability) res.YapaServiceAvailability = {
            IsAvailable: false,
            IsIndoor: false,
            IsTurknetStatusActiveForSantral: false,
            Description: null
        };
